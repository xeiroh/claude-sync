...
class TestSyncManager:
    ...
    def test_install_claude_code_remote_npm(self, mock_check, sync_manager):
        ...
        # Verify correct npm command was used
        call_args = mock_ssh.exec_command.call_args[0][0]
        assert "npm install -g @anthropic-ai/claude-code@2.0.5" in call_args
    ...
    def test_install_claude_code_remote_native(self, mock_check, sync_manager):
        ...
        assert success is True
        # Verify correct native command was used
        call_args = mock_ssh.exec_command.call_args[0][0]
        assert "curl -fsSL https://claude.ai/install.sh" in call_args
        assert "bash -s 2.0.5" in call_args
